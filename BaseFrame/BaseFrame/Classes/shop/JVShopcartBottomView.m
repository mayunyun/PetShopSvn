...
- (void)configureShopcartBottomViewWithTotalPrice:(float)totalPrice totalCount:(NSInteger)totalCount isAllselected:(BOOL)isAllSelected {
    ...
    if (self.settleButton.isEnabled) {
        [self.settleButton setBackgroundColor:UIColorFromRGB(0xeb6876)];
        [self.deleteButton setBackgroundColor:UIColorFromRGB(0xeb6876)];
//        [UIColor colorWithRed:0.918  green:0.141  blue:0.137 alpha:1]]
...
- (void)renderWithTotalPrice:(NSString *)totalPrice {
    ...
    [attributedString addAttributes:@{NSForegroundColorAttributeName:[UIColor redColor]} range:[self.totalPriceLable.text rangeOfString:totalPrice]];
