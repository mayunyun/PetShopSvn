...
- (void)setUpBase
{
    ...
    if (statusbarHeight>20) {
        tY = ([self.navigationController isNavigationBarHidden] == YES) ? 44 : DCNormalTitleViewH + 44;
    }
...
{
//    if (_selectBtnBlock) {
//        self.selectBtnBlock = ^[(NSInteger index) {
//            
//        }] ;
//    }
    _pregressView.isStretch = NO;
    ...
    NSString * str = [NSString stringWithFormat:@"%zd",buttonTag];
    NSDictionary *dict = @{@"tag":str};
    //创建通知
    NSNotification *notification =[NSNotification notificationWithName:@"typeUpData" object:nil userInfo:dict];
    //通过通知中心发送通知
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSString * str = [NSString stringWithFormat:@"%zd",tagI];
    NSDictionary *dict = @{@"tag":str};
    //创建通知
    NSNotification *notification =[NSNotification notificationWithName:@"typeUpData" object:nil userInfo:dict];
    //通过通知中心发送通知
